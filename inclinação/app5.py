...
def correct_image_orientation(image):
    ...
    print("Número de rostos detectados:", len(faces))  # Adicionar instrução de impressão
    ...
    print("Pontos-chave do rosto:", face_landmarks)  # Adicionar instrução de impressão
    ...
    print("Ângulo de inclinação:", angle)  # Adicionar instrução de impressão
...
def crop_background(image):
    try:
        ...
        if len(face_locations) == 0:
            ...
            x1 = max(0, left - int(face_width * (left_factor - 2) / 2))
...
def extract_text():
    try:
        ...
        # Recortar o documento antes de corrigir a inclinação
        ...
        cv2.imwrite(cropped_image_path, cropped_document)
        
        # Corrigir a inclinação da imagem
        rotated_image = correct_image_orientation(cropped_document)
        
        # Extrair texto da imagem
        extracted_text = extract_text_from_image(rotated_image, cropped_image_path)
        
        # Salvar a imagem corrigida
        corrected_image_path = os.path.join(output_folder, 'corrected_image.jpg')
        cv2.imwrite(corrected_image_path, rotated_image)
        
        return jsonify({'text': extracted_text, 'corrected_image_path': corrected_image_path, 'cropped_image_path': cropped_image_path})
